...
\begin{definition} \textbf{Execution of controllers}:
	In the execution time, a controller $ \mathcal{C}=(\mathcal{V},\mathcal{K},x) $ acts as a function that maps the current state to a set of feasible actions. Given the current state $ s $, the output $ \mathcal{C}(s) $ is determined in two steps:
