...
\end{itemize}
	...
	In the execution of $ \mathcal{K}(x^+)(s) $, the process above would be repeated and then a sub-controller of $ \mathcal{K}(x^+) $ would be executed. So obviously it's recursive algorithm which ends when a simple controller is reached.
