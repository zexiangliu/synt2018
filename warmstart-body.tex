...
Recall that the controller in Definition \ref{def:cont} has a tree structure whose nodes consist of controllers (non-leaf nodes) and simple controller (leaf node): all those non-leaf nodes are computed by fixed-point based algorithms for some intermediate specifications (different from \eqref{phi}; see Appendix \ref{app:basic-alg}); all the leaf nodes are computed by algorithm either \eqref{eqn:pre} or \eqref{win-inv}. In Section \ref{sec:method}, we show that each algorithm mentioned above has a corresponding patching algorithm which could restrict an existing controller resulting from it to a smaller action space with a stronger specification \footnote{For specifications $ \phi $ and $ \psi $, ``$ \phi $ is stronger than $ \psi $"  means that whenever $ \phi $ is satisfied, $ \psi $ is satisfied. Here a stronger specification always refers that the target set $ Z $ in the specification of a fixed-point based algorithm shrinks (if its specification has such a term, see Section \ref{sec:method}).}. It allows us to offer a solution to Problem \ref{prob} by patching the sub-controllers of a controller resulting from algorithm \eqref{win-phi} one by one.

Furthermore, since there is an order between the nodes in each layer of the tree structure (recall that $ \mathcal{K} $ is a list), they should be modified in the order they are generated in the original algorithms. Also, the inputs for each fixed-point based algorithm needs to be retrieved. So to modify a controller, we basically re-run the original fixed-point based algorithm, but replace the other fixed-point based algorithms it calls internally to be their corresponding patching algorithms with the original inputs, changed term in specification and the existing controller of that node. It's easy to see that the correctness of a patching algorithm depends on the correctness of the patching algorithms called inside it. In the end, we show that as long as we can modify leaf nodes (simple controllers) and retrieve the original inputs for each fixed-point based algorithm correctly, the winning set corresponding to the modified controller is exactly the same as that resulting from re-synthesizing a new controller.
...
\begin{definition}
	...
	%winning set $ W_1 $ and $ W_2 $
	 {\color{purple} over sets $D_1$ and $D_2$\footnote{In this work, $ D_1 $ and $ D_2 $ are always equal to the winning sets corresponding to $ \mu_1 $ and $ \mu_2 $. In general, they could be larger than the winning sets dependent on the type of specification.}}
