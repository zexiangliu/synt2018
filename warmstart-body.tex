...
For two sets $ A $ and $ B $, the set difference is denoted by $ A-B = A\cap \neg B $, where $ \neg B $ represents the complement of set $ B $. A function or a map $ F: C\rightarrow D $ {\color{purple} corresponds to  the set} $ \{(c,F(c)):c\in C, F(c)\in D \} $. {\color{purple} Not sure it is really useful to define functions or maps here, the terms are barely used in the rest of the document (function x4, map x1)} 

\subsection{Augmented Finite Transition Systems}

\begin{definition}
	An \textbf{augmented finite transition system} (AFTS) \cite{Nilsson2017} is a tuple $ T = (Q,U,\rightarrow_T, G,AP,h_Q) $, where $ Q $ is a set of states, $U$ is a set of actions (control inputs), $ \rightarrow_T\in Q\times U \times Q $ is {\color{purple} is a transition relation} between states under specific actions, $ G: 2^U\rightarrow 2^{2^Q} $ is a progress group map, $ AP $ is a finite set of atomic propositions, and $ h_Q: Q\rightarrow 2^{AP} $ is a labeling function, which maps states to the set of atomic propositions they preserve {\color{purple} verify}. {\color{purple} Maybe a few words on the progress group maps, which is a non-standard notion: A set $G \in G(U)$ is called a \emph{progress group under the action set $U$}, and it is related to the following semantic notion: the system cannot remain indefinitely within
$G$ by exclusively choosing actions from $U$. This restricts the behaviors allowed by the transition relation.}
\end{definition}
...
The other operators can be derived as following: (vi) $ \phi \wedge \psi = \neg (\neg \phi \vee \psi) $, (v) $ \phi \implies \psi = \neg \phi \wedge \psi $, (vi) $ \Diamond \phi = True \mathbf{\ U\ } \psi $, (vii) $ \Square \phi = \neg \Diamond \neg \phi $.

An $\omega$-word is an infinite sequence in $ 2^{AP}$.  The satisfaction of an LTL specification $ \phi $ by an $\omega$-word $ w = w(0)w(1)w(2)\dots $, written as $ (w,i) \models \phi $, is defined inductively as follows:
\begin{itemize}
	\item if $ \phi \in AP $, $ (w,i)\models p $ iff $ p\in w(i) $ 
	\item $ (w,i)\models \neg \phi $ iff $ (w,i)\not\models \phi $
	\item $ (w,i)\models  \phi \vee \psi $ iff $ (w,i)\models \phi $ or $ (w,i)\models \psi $
	\item $ (w,i) \models \bigcirc \phi $ iff $ (w,i+1) \models \phi $
	\item $ (w,i)\models \phi \mathbf{\ U\ } \psi $ iff $\exists j\geq i  $ s.t. $ (w,j)\models \psi $ and $ (w,k)\models \phi, \forall k\in [i,j) $
\end{itemize} 
...
\begin{align}
\phi = \Square A \wedge \Diamond \Square B \wedge \left( \bigwedge_{i\in I} \Square \Diamond G^i\right)\label{phi}
\end{align}
{\color{purple} for subsets $A$, $B$, $G^i \subseteq Q$ defining atomic propositions. Such specifications can express properties of \emph{safety} ($\Square A$), \emph{stability} ($\Diamond \Square B$) and \emph{progress} ($\Square \Diamond G^i$). (In the original AFTS paper: "invariance, persistence and reccurence")}

\begin{definition}
	{\color{purple} A} \textbf{winning set} of the specification $ \phi $ over an AFTS $ T = (Q, U, \rightarrow_T, G, AP, h_Q) $, written as $ W_{\phi} $, is the largest subset of $ Q $ s.t. if the system $ T $ is initially in $ W_{\phi} $, specification can be enforced.
\end{definition}
...
\begin{definition}
	For the AFTS $ T=\{Q,U,\rightarrow_T,G,AP,h_Q\} $, a \textbf{controller} $ \mathcal{C} $ for a winning set $ \text{Win}^T(\phi) $ is a tuple $ (\mathcal{V},\mathcal{K},x) $, where $ \mathcal{V} $ is a partially ordered subset of $ 2^Q $, $ \mathcal{K} $ is a partially ordered set of controllers or simple controllers, i.e. so-called sub-controllers of $ \mathcal{C} $, and $ x $ is an internal variable that saves the index of sub-controllers called last time. 
	\label{def:cont}
\end{definition}

 {\color{purple} Winning sets and controllers} for specifications in the form of \eqref{phi} are computed iteratively via the following fixed-point based algorithm {\color{purple} (see \cite{Nilsson2017})}. Each fixed-point operator has two outputs: the winning set (1st) and controller (2nd). When a fixed-point operator appear in set calculation, it refers to its first output, i.e. the winning set.
...
\begin{align}
& [V_{\infty},\mathcal{C}] =\  \text{Win}_{\exists, \forall}^{T,U}\left(\Square A \wedge \Diamond \Square B \wedge \left( \bigwedge_{i\in I} \Square \Diamond G^i\right)\right)\\
...
\ \ [V_{k+1}, \mathcal{C}_{k+1}]=\text{Win}_{\exists,\forall}^{T} ((B \mathbf{\ U\ }Z_{k+1}) \vee \Square (B\wedge ( \bigwedge_{i\in I}\Diamond G^i))\\
...
The winning set {\color{purple} that results from} \eqref{win_phi} is {\color{purple} equal to} $ V_{\infty} $, i.e. {\color{purple} the limit of the expanding sequence $V_k$}. $ \mathcal{C} $ is the controller corresponding to the winning set $ V_{\infty} $. The building blocks of the above algorithm are as follows:
\begin{align}
&[W_{\infty},\mathcal{C}]= \ \text{Win}_{\sharp,\forall}^{T,U}((B\mathbf{\ U\ }Z)\vee \Square (B\wedge(\bigwedge_{i\in I}\Diamond G^i)) )\\
=&\begin{cases}
W_0 = Q,\ \mathcal{V}=\{\},\ \mathcal{K}=\{\}\\
while\ W_{k+1}\not= W_k\\
\ \ \ \ Z_{k+1}^i = Z\cup (B\cap G^i\cap \text{Pre}_{\sharp, \forall}^{T,U}(W_k))\\
\ \ \ \ [X^i, \mathcal{C}^i]= \text{Win}_{\sharp,\forall}^{T,U} (B\mathbf{\ U\ }Z_{k+1}^i), \forall i \in I\\
\ \ \ \ W_{k+1} = \bigcap_{i\in I} X^i,\ \mathcal{K}(i) = \mathcal{C}^i, \forall i\in I\\
\mathcal{V}(1)=W_k,\ \mathcal{V}(i+1) = B\cap G^i, \forall i\in I\\
W_{\infty} = W_k,\ \mathcal{C} = (\mathcal{V},\mathcal{K},x)
\end{cases}\label{win_interm}
\end{align}
\begin{align}
&[X_{\infty},\mathcal{C}]= \text{Win}_{\sharp,\forall}^{T,U} (B\mathbf{\ U\ }Z)\\
=&\begin{cases}
X_0 = \emptyset,\ \mathcal{V}=\{\},\ \mathcal{K}=\{\}\\
while\ X_{k+1}\not= X_k\\
\ \ \ \ [V^1_k,C^1_k]=
\text{Pre}_{\sharp,\forall}^{T,U}(X_k)\\
\ \ \ \  [V^2_k,C^2_k]=\text{PGPre}_{\sharp,\forall}^{T,U}(Z\cup(B\cap V^1_k),B)\\
...
\ \ \ \ \ \ \mathcal{V}(k)=V_k,\ \mathcal{K}(k)=\mathcal{C}_k,\ k=k+1\\
\mathcal{C} = (\mathcal{V},\mathcal{K},x)
\end{cases}\label{win_pgpre}
\end{align}
\begin{align}
&[W,\mathcal{C}]=\text{Pre}_{\sharp_1,\sharp_2}^{T,U}(V)\\
=&\begin{cases}
W = \{q_1\in Q: \sharp_1 (u\in U) \sharp_2 (q_2\ s.t.\ (q_1, u, q_2) \in \rightarrow_T), q_2\in V\}\\
...
The winning sets {\color{purple} resulting from} \eqref{win_interm}, \eqref{win_until}, \eqref{win_inv} are $ W_{\infty} $, $ X_{\infty} $ and $ Y_{\infty} $. {\color{purple} $\text{Pre}_{\sharp_1,\sharp_2}^{T,U}$ is a one-step reachability operator, and $\text{Inv}_{\sharp}^{U,G}(Z,B)$ computes $Y_\infty \subseteq (G\cap B) - Z$ from where the state can be controlled (with actions in $U$) to either remain inside $Y_\infty$ or reach $Z$, but because $G$ is a progress group under $U$, remaining indefinitely in $G$ is impossible and therefore $Z$ is
...
\begin{definition} \textbf{Execution of controllers}:
	In the execution time, a controller $ \mathcal{C}=(\mathcal{V},\mathcal{K},x) $ acts as a function that maps the current state $ u(t) $ to a set of feasible actions according to the following rules:
	
	\begin{displaymath}
	\mathcal{C}(u(t)) = \mathcal{K}(x(t))(u(t))
	\end{displaymath}
\quad	For controllers returned by \eqref{win_phi},\eqref{win_until},\eqref{win_inv}:
	\begin{displaymath}
		x(t) =	\begin{cases}
		\text{arg}\min_{x\leq x(t-1)}\{u(t)\in \mathcal{V}(x)\},\ \ if\ x(t-1)\not=0\\
		\text{arg}\min_{x}\{u(t),\ \ otherwise
		\end{cases}
	\end{displaymath}
\quad For controllers returned by \eqref{win_interm}:
	\begin{displaymath}
		x(t) = \begin{cases}
		x(t-1)+1 \mod\vert\mathcal{K}\vert,\ \ if\ u(t)\in \mathcal{V}(x(t-1)+1)\\
		x(t-1),\ otherwise
		\end{cases}
	\end{displaymath}
	Namely, internal variable $ x(t) $ will be updated each time $ \mathcal{C} $  is called. The output action sets of $ \mathcal{C} $ is equal to the output of the $ x(t)^{th} $ sub-controller in the partially ordered set $ \mathcal{K} $ given the input $ u(t) $, i.e. $ \mathcal{K}(x(t))(u(t)) $. The same process will happen recursively in the execution of $ \mathcal{K}(x(t))(u(t)) $, until a simple controller is reached. 
...
{\color{blue} don't use b.w., use between. in general, it is better writing practice to avoid abbreviations as much as possible}

	\subsection{Problem Statement}

Consider an AFTS $ T = (Q,U,\rightarrow_T,G,AP,h_Q) $ and a LTL specification $ \phi = \Square A \wedge \Diamond \Square B \wedge \left( \bigvee_{i\in I} \Square \Diamond G^i\right) $. Compute its winning set and the winning control strategy (or controller) via fixed-point operator \eqref{win_phi}, denoted by $ W_{\phi} $ and $ \mu_{\phi} $ (or $ \mathcal{C}_{\phi} $).

Now disable some actions $ U_d \subseteq U$, i.e. getting a new AFTS $ \widehat{T} = (Q,U-U_d,\rightarrow_T,G,AP,h_Q) $ and re-compute the winning set and control strategy, denoted by $ \widehat{W}_{\phi} $ and $ \widehat{\mu}_{\phi} $ (or controller $ \widehat{\mathcal{C}}_{\phi} $), via the same fixed-point operator.

\begin{definition}
	For two control strategies $ \mu_1 $ and $ \mu_2 $ with winning set $ W_1 $ and $ W_2 $, $ \mu_{1} \subseteq \mu_{2}$ if $ \mu_{1} (\{(s_i,a_i)\}_{i=1}^{n-1}, s_n) $ is a subset of $ \mu_{2}(\{(s_i,a_i)\}_{i=1}^{n-1}, s_n) $, for all feasible $ n $ and all transition trajectories $ q =(s_1,a_1),(s_2,a_2),...,(s_{n-1},a_{n-1}) $ satisfying $ s_1\in W_{1} $, $ a_k \in \mu_{1} (\{(s_i,a_i)\}_{i=1}^{k-1}, s_k) $.
\end{definition}

\begin{theorem}
	For $ W_{\phi}, \mu_{\phi}, \widehat{W}_{\phi}, \widehat{\mu}_{\phi} $ defined above, $ \widehat{W}_{\phi} \subseteq W_{\phi}  $ and $ \widehat{\mu}_{\phi} \subseteq \mu_{\phi}$\label{thm: 1}.
\end{theorem}
...
\begin{problem}
	Given an AFTS $ T = (Q,U,\rightarrow_T,G,AP,h_Q) $, a LTL specification $ \phi = \Square A \wedge \Diamond \Square B \wedge \left( \bigvee_{i\in I} \Square \Diamond G^i\right) $, the winning set $ W_{\phi} $ and controller $ \mathcal{C}_{\phi} $, if a set of actions $ U_d\subseteq U $ in $ T $ is unavailable, find the new winning set and controller based on $ W_{\phi} $ and $ \mathcal{C}_{\phi} $.
\end{problem}
...
\end{remark}
Due to the easy conversion between  $ \mathcal{C} $ and $ T(\mathcal{C}) $, we modify a simple controller $ \mathcal{C} $ by modifying $ T(\mathcal{C}) $ and then converting $ T(\mathcal{C}) $ to $ \mathcal{C} $, with the advantages that $ T(\mathcal{C}) $ contains the necessary transition information and is easier to manipulate. 

Assume the AFTS $ T = (Q,U,\rightarrow_T, G, AP, h_Q) $ is the abstraction of the system that needs to be controlled. Assume that $ T_{\mathcal{C}}=(Q_{\mathcal{C}}, U_{\mathcal{C}},\rightarrow_{T_{\mathcal{C}}})$ is a transition system corresponding to a simple controller $ \mathcal{C} $. 

Firstly let's define several operation for the FTS $ T(\mathcal{C}) $

\subsubsection{transitions removing}
remove all transitions contained in $ E $ from $ T_{\mathcal{C}} $:
\begin{align}
\rightarrow_{T_{\mathcal{C}}} - E
\end{align}
where $ E = \{(s_1,a,s_2): s_1,s_2\in Q_{\mathcal{C}}, a\in U_{\mathcal{C}}\} $. It's a set difference between $ \rightarrow_{T_{\mathcal{C}}} $ and $ E $. For simplicity, it is also denoted by $ T_{\mathcal{C}}\backslash E $

There are three convenient variations of this operator:
\begin{itemize}
	\item $ T_{\mathcal{C}} \backslash (E,*) \equiv T_{\mathcal{C}}\backslash \{(s_1,a,s_2)\in \rightarrow_{T_{\mathcal{C}}}: (s_1,a)\in E)\}$, where $ E \subseteq Q_{\mathcal{C}}\times U_{\mathcal{C}} $.
	\item $ T_{\mathcal{C}} \backslash (*,U_d,*) \equiv T_{\mathcal{C}}\backslash \{(s_1,a,s_2)\in \rightarrow_{T_{\mathcal{C}}}: a\in U_d \}$, where $ U_d\subseteq U_{\mathcal{C}} $.
	\item $ T_{\mathcal{C}} \backslash (Q_d,*,*)\equiv T_{\mathcal{C}}\backslash \{ (s_1,a,s_2)\in \rightarrow_{T_{\mathcal{C}}}: s_1\in Q_d \}$, where $ Q_d \subseteq Q_{\mathcal{C}} $ 
\end{itemize}

\subsubsection{transition adding} given $ E \subseteq Q \times U \times Q$, replace $ \rightarrow_{T_{\mathcal{C}}} $ with $ \rightarrow_{T_{\mathcal{C}}}\cup E $, denoted for simplicity by
\begin{align}
T_{\mathcal{C}}\cup E
\end{align}
...
\subsubsection{null node seeking}
\begin{itemize}
	\item $ Vac(T_{\mathcal{C}})$: return the set $\{s_1\in Q_{T_{\mathcal{C}}}: \nexists (s_1,a,s_2)\in \rightarrow_{T_{\mathcal{C}}},\forall a\in U_{T_{\mathcal{C}}}, \forall s_2 \in Q_{T_{\mathcal{C}}}\} $, i.e. all the nodes whose out-degree is zero.
\end{itemize}

\subsubsection{state-action pre}
\begin{itemize}
	\item $  \widehat{Pre}^{T_{\mathcal{C}},U}_{\exists,\exists}(S_2)$: return the set $ \{(s_1,a): (s_1,a,s_2)\in \rightarrow_{T_{\mathcal{C}}}, s_2\in S_2\} $, i.e. all the edges pointing into nodes in $ S_2 $
\end{itemize}
...
\ \ \ \ \ \  Z_\infty = Z_\infty\cup \mathcal{V}(k),\ \widehat{Z}_{\infty} = \widehat{Z}_{\infty} \cup \widehat{\mathcal{V}}(k),\ k=k+1\\
...
Assume that $ W $ and $ \mathcal{C}=(\mathcal{V},\mathcal{K},x) $ are the winning set and controller resulting from $\text{Win}_{\exists,\forall}^{T,U}((B\mathbf{\ U\ }Z)\vee \Square(B\wedge (\bigwedge_{i\in I} \Diamond G^i)) $. Given that $ \widehat{Z}\subseteq Z $ and $ \widehat{U}\subseteq U $, modify $ W $ and $ \mathcal{C} $ for $\text{Win}_{\exists,\forall}^{T,U}((B\mathbf{\ U\ }\widehat{Z})\vee \Square(B\wedge (\bigwedge_{i\in I} \Diamond G^i)) $. The patching operator for \eqref{win_interm}, written as $\overline{\text{Win}}^{T, U}_{\exists,\forall (\psi)}  $ is 
\begin{align}
...
Z_{\infty}^i = Z\cup (B\cap G^i\cap \text{Pre}_{\exists, \forall}^{T,U}(W_0))\\
\widehat{Z}_{0}^i = \widehat{Z}\cup (B\cap G^i\cap \text{Pre}_{\exists, \forall}^{T,U-U_d}(W_0))\\
...
\ \ \ \ \widehat{Z}_{k+1}^i = \widehat{Z} \cup (B\cap G^i\cap \text{Pre}_{\exists, \forall}^{T,U-U_d}(\widehat{W}_k))\\
...
\begin{displaymath}
 \text{Win}_{\exists, \forall}^T\left(\Square A \wedge \Diamond \Square B \wedge \left( \bigwedge_{i\in I} \Square \Diamond G^i\right)\right)
\end{displaymath}

 Assuming that $ \vert \mathcal{K}\vert = n $. For patching purpose, we need some extra information: the partially ordered sets of winning sets and controllers returned by $\text{Pre}_{\exists,\forall}^{T,U}(V_k)$ in \eqref{win_phi}, i.e. $ [\mathcal{V}_1(k),\mathcal{K}_1(k)]=\text{Pre}_{\exists,\forall}^{T,U}(V_k)$;  the partially ordered sets of winning sets and controllers returned by $ \text{PGPre}_{\exists,\forall}^{T}(V_k, B) $ in \eqref{win_phi}, i.e. $ [\mathcal{V}_2(k), \mathcal{K}_2(k)]= \text{PGPre}_{\exists,\forall}^{T}(V_k, B)$ ($ k=1,..., n $) ; the controller $ \mathcal{C}_{Inv} $ returned by $ \text{\text{Win}}^{T,U}_{\exists, \forall} ((A\mathbf{U}\emptyset)\vee \Square (A\wedge \Diamond Q)) $ in the first line of \eqref{win_phi}. To restrict action space to $ \widehat{U} = U-U_d $, the patching operator for \eqref{win_phi} is 
\begin{align}
...
$ \widehat{\mathcal{C}}^0_0 = (\{\widehat{W},\{s_1\},\{s_2,s_3\}\},\{\widehat{\mathcal{C}}^1_0, \widehat{\mathcal{C}}^1_1\},x^0) $;
...
\begin{figure}
	...
	\caption{ Transition graph of internal variables. The update rules are the same as in Figure \ref{fig:eg1}, but transition conditions change. }
	\label{fig:xupdate2}
\end{figure}

For the initial state $ s_1 $ and initial internal variables $ (0,0,0,0) $, only one trajectory available under control of $\widehat{\mathcal{C}} $ is $ s_1,s_2,s_3,s_2,s_1,...$, where the sequence of actions is $ b,f,g,c,... $. The trajectory doesn't visit $ s_4 $, for the system isn't able to jump to $ A $ from $ s_4 $ after action $ e $ is removed.
...
\begin{table}
	...
	\begin{tabular}{lllllll}
		...
		\hline 
		$ \sharp $trans & $ 100\% $ & $ 96.57\% $ & $ 81.22\% $ & $ 60.55\% $ & $ 39.67\% $ & $ 19.00\% $\\
...
\begin{table}
	\caption{Comparison of re-synthesis time and patching time under random action profiles. The first row is number of unavailable actions. For each number, choose 10 random sets of unavailable actions. The last two rows are the average time used for re-synthesizing and patching.}
