...
Note that although we are only interested in the specification in the form of \eqref{phi}, to compute its winning set and controller, several fixed-point based algorithms for specifications not in the form of \eqref{phi} are called by \eqref{win-phi} internally. The output winning sets and controllers of those fixed-point algorithms are consistent with Definition \ref{def:winset} and \ref{def:cont}, which are usually stored as sub-controllers in the output controller of \eqref{win-phi}. 
...
In Section \ref{sec:method}, we show that each fixed-point based algorithm \eqref{win-phi}-\eqref{win-inv} 
