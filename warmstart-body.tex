...
\begin{theorem}
	$ \widehat{W} $ and $ \widehat{C} $ returned by \eqref{patch_pre} would be the same as outputs resulting from $ \text{Pre}_{\exists,\forall}^{T,\widehat{U}}(\widehat{V})$.	
\end{theorem}
...
\begin{theorem}
$ \widehat{Y} $ and $ \widehat{C} $ returned by \eqref{patch_inv} would be the same as outputs resulting from $ \text{Inv}_{\exists}^{D,G}(\widehat{Z},B) $.	
\end{theorem}
