...
Linear Temporal Logic (LTL) is utilized to describe the desired behaviors achieved in an AFTS or the concrete system, which we call it a specification of our control synthesis problem. It has logic operators (negation $ \neg $, conjunction $ \wedge $, disjunction $ \vee $), and temporal operators (next $ \bigcirc $, always $ \Square $, eventually $ \Diamond $ and until $ \mathbf{\ U\ }$).
