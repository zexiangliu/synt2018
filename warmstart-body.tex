...
\begin{definition} \textbf{Execution of controllers}:
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\end{cases}
			\end{displaymath}
			\item  if $ \mathcal{C} $ results from \eqref{win-interm}: %{\color{purple} just $x \mod ..$? It seems that $x$ may remain constant with this definition, are you sure that this is correct?}{\color{teal} there's $ +1 $ after mod}
			\begin{displaymath}
			x^+ = \begin{cases}
