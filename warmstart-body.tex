...
Note that although we are only interested in the specification in the form of \eqref{phi}, to compute its winning set and controller, we need the fixed-point based algorithms for some other specifications, which may not be in the form of \eqref{phi}. All those algorithms are also given in Appendix A, which output the winning sets and controllers consistent with Definition \ref{def:winset}, \ref{def:simp} and \ref{def:cont}. 
...
In Section \ref{sec:method}, we show that for winning sets and controllers computed by fixed-point based algorithms \eqref{win-phi}-\eqref{win-inv}, controllers can be modified  
