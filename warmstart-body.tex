...
Recall that the controller in Definition \ref{def:cont} has a tree structure whose nodes consist of controllers (non-leaf nodes) and simple controllers (leaf nodes): all those non-leaf nodes are computed by fixed-point based algorithms for some intermediate specifications (different from \eqref{phi}; see Appendix \ref{app:basic-alg}); all the leaf nodes are computed by algorithm \eqref{eqn:pre} or \eqref{win-inv}. In Section \ref{sec:method}, we show that each algorithm mentioned above has a corresponding patching algorithm which could restrict an existing controller resulting from it to a smaller action space with a stronger specification \footnote{In general, for specifications $ \phi $ and $ \psi $, ``$ \phi $ is stronger than $ \psi $"  means that whenever $ \phi $ is satisfied, $ \psi $ is satisfied. Here a stronger specification always refers that the target set $ Z $ in the specification of a fixed-point based algorithm shrinks (if its specification has such a term, see Section \ref{sec:method}).}. It allows us to offer a solution to Problem \ref{prob} by patching the sub-controllers of a controller resulting from algorithm \eqref{win-phi} one by one.

Furthermore, since there is an order between the nodes in each layer of the tree structure (recall that $ \mathcal{K} $ is a list), they need to be modified in the order they are generated in the original algorithms. Also, the inputs for each fixed-point based algorithm needs to be retrieved. So to patch a controller resulting from a specific fixed-point based algorithm, we basically re-run that algorithm, but replace the fixed-point based algorithms it calls internally to be their corresponding patching algorithms with the original inputs, changed term in specification and the existing controller of that node. Obviously the correctness of this patching algorithm depends on the correctness of the patching algorithms called by it. In the end, we show that as long as we can patch leaf nodes (simple controllers) and retrieve the original inputs for each fixed-point based algorithm correctly, the winning set corresponding to the modified controller is exactly the same as the winning set resulting from re-synthesizing the controller.
...
Given an AFTS $ T = (Q,U,\rightarrow_T,G,AP,h_Q) $ and {\color{blue} an} LTL specification $ \phi $, compute its winning set $ W_{\phi} $ and the winning control strategy $ \mu_{\phi} $  via one of the fixed-point based algorithms. Now disable the set of actions $ U_d \subseteq U$ in the AFTS $ T $, which results in one new AFTS $ \widehat{T} = (Q,U-U_d,\rightarrow_{\widehat{T}},G,AP,h_Q) $, and re-compute the winning set and control strategy, denoted by $ \widehat{W}_{\phi} $ and $ \widehat{\mu}_{\phi} $, for a specification the same as or stronger than $ \phi $ over $ \widehat{T} $ via the same algorithm. Then, we have the following theorem:
...
\emph{Proof:} (i) For any state $ s\in \widehat{W}_{\phi} $, if the system $ \widehat{T} $ starting from $ s $ could be controlled under an infinite sequence of actions and satisfy the specification $ \phi $, we can apply the same sequence of actions over $ T $ and get the same trajectory. That implies $ s\in W_{\phi} $, because the winning sets resulting from the fixed-point based algorithms are sound and complete\footnote{Proofs of The soundness and completeness of \eqref{win-phi}, \eqref{win-interm} and \eqref{win-inv} are given in the Appendix A of \cite{Nilsson2017}. The soundness and completeness of \eqref{win-pgpre}, \eqref{win-inv} and \eqref{eqn:pre} are trivial.}. Therefore, $ \widehat{W}_{\phi}\subseteq W_{\phi} $. (ii) Given the winning set $ W $, $ Pre_{\exists,\forall}^{T,U}(W)$ restricted to $ W $ gives the simple controller. By the definition of $ Pre^{T,U}_{\exists,\forall} $ in \eqref{eqn:pre}, the control strategies implemented in the simple controller are sound and complete as long as $ W $ is sound and complete. Thus control strategies $ \widehat{\mu}_\phi $ and $ \mu_\phi $ are sound and complete. Since $ \widehat{\mu}_\phi $ has a smaller action space with the same or a stronger specification than $ \mu_\phi $, every sequence of actions given by $ \widehat{\mu}_\phi $ works for $ T $ too and should be given by $ \mu_\phi $ due to the completeness. That implies that $ \widehat{\mu}_\phi \subseteq \mu_\phi $.
%$\qedhere$

Theorem \ref{thm: 1} says that the winning set of the existing controller always shrinks after a set of actions are disabled, which agrees with our intuition. Furthermore, it shows the containment relationship in simple controllers before and after changes in the action space, which reveals the possibility of patching the simple controllers instead of re-synthesizing them. This is significant because according to the data structure in Definition \ref{def:cont}, a controller is always built on simple controllers, i.e. the leaf nodes in the tree structure. If we can patch the leaf nodes, it's possible to patch the parent nodes of the leaves, and so forth. 

In Section \ref{sec:patch-simple}, we will firstly discuss the patching algorithms for the simple controllers, and then in Section \ref{sec:patch-cont} we'll show how to patch the other nodes built on the leaf nodes, and finally patch the root node, i.e the controller resulting from \eqref{win-phi}. 
