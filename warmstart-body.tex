...
\widehat{Y}=Y_k,\ [-,\widehat{\mathcal{C}}] = \overline{\text{Pre}}_{\exists,\forall}^{T,D}(\mathcal{C}, (Y\cup Z )- (\widehat{Y}\cup \widehat{Z}),\emptyset)\\  \widehat{\mathcal{C}}\ restricted\ to\ \widehat{Y},
%A(\widehat{\mathcal{C}}) = T_{k} 
\end{cases}\label{patch-inv}
\end{align}}
where $ \Delta Z = Z - \widehat{Z} $, $ E_0 = \{(s_1,a,s_2)\in \rightarrow_{T}: s_1\in Y_0, a\in D, s_2 \in Q\} $, $ \text{Pre}_{\forall,\exists}^{T_k,D}(\Delta Y_k) = \{q_1: \forall u\in D, \exists q_2\in \Delta Y_k, s.t.\ (q_1, u, q_2) \in \rightarrow_{T_k} \} $. $ \widehat{Y} $ is the modified winning set, and the controller $ \widehat{\mathcal{C}} $ can either be recovered from $ T_k $ or be computed as in \eqref{patch-inv}.
