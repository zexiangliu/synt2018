...
A \emph{trajectory} of an AFTS $T$ is an infinite sequence $ s = \{s(n)\}_{n=0}^{\infty} $ 
...
Given a trajectory $ s = \{s(n)\}_{n=0}^{\infty} $ 
{\color{purple} Given a trajectory $\{s(n)\}_{n=0}^{\infty} $ }
of an AFTS, the \textit{$\omega$-word} corresponding to $ s $ {\color{purple} $\{s(n)\}_{n=0}^{\infty} $} is $ h_Q(s) =  \{h_Q(s(n))\}_{n=0}^{\infty} $ {\color{purple} $\{h_Q(s(n))\}_{n=0}^{\infty}$}, where $ h_Q $ is the labeling function of the AFTS.  Given a LTL specification $ \phi $, we say that $ s\models \phi $ {\color{purple} $\{s(n)\}_{n=0}^{\infty} \models \phi $} if and only if $ h_Q(s) \models \phi $ {\color{purple} $\{h_Q(s(n))\}_{n=0}^{\infty} \models \phi $}.
...
\begin{definition}
	A \textbf{simple controller} for a winning set $ W $ over an AFTS $ T=\{Q,U,\rightarrow_T,G,AP,h_Q\} $ is a function $ \mathcal{C}: $ W $ \rightarrow 2^U $, i.e. a memoryless controller. A simple controller maps the current state to a set of actions under which the specification in the winning set remains guaranteed. {\color{purple} The problem here is that a simple controller is not always enough to enforce a specitifcation; here I wouldn't make the link between the simple controller and specifications or winning sets; it is just the definition of what a simple 
controller is, not necessarily a simple controller that has been constructed to enforce a specification; so I would say "a simple controller for a subset $D \subset Q$ over an .."}
	\label{def:simp}
\end{definition}
...
\begin{definition}
	For the AFTS $ T=\{Q,U,\rightarrow_T,G,AP,h_Q\} $, a \textbf{controller} $ \mathcal{C} $ for a winning set $ \text{Win}^T(\phi) $ is a tuple $ (\mathcal{V},\mathcal{K},x) $, where $ \mathcal{V} $ is a list of subsets of $ Q $, $ \mathcal{K} $ is a list of controllers or simple controllers, i.e. so-called {\color{purple} which we refer to as} sub-controllers of $ \mathcal{C} $, and $ x $ is an internal variable that saves the index of sub-controllers called last time. {\color{purple} Same remark as with the simple controllers: no need for a connection to winning sets here. Also, I still believe that the transition graph is lacking in this definition: without it, we don't know how to update $x$}
	\label{def:cont}
\end{definition}
