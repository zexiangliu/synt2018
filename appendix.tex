...
In this appendix, we present algorithms from \cite{Nilsson2017} to compute the winning set for a specification in the form \eqref{phi}. In addition to the winning set, these algorithms provide an explicit construction of the control implementation as defined in Definition \ref{def:cont}. 

The most general algorithm that compute the winning set and controller for specifications in the form of \eqref{phi} is given in \eqref{win-phi}. The winning set {\color{purple} that results from} \eqref{win-phi} is {\color{purple} equal to} $ V_{\infty} $, i.e. {\color{purple} the limit of the expanding sequence $V_k$}. $ \mathcal{C} $ is the controller corresponding to the winning set $ V_{\infty} $. 
...
\begin{tabular}{cc}
	\begin{minipage}{0.5\textwidth}
		...
		V_0 = \emptyset,\ \mathcal{V}=\{\},\ \mathcal{K} = \{\},\  {\color{purple} k=0}\\
	...
	\begin{minipage}{0.5\textwidth}
		...
		=\begin{cases}
		W_0 = Q,\ \mathcal{V}=\{\},\ \mathcal{K}=\{\}\\ {\color{purple} k=0}\\
...
\begin{table}
	...
	\begin{tabular}{cc}
		\begin{minipage}{.5\textwidth}
			\begin{align}
			[X_{\infty},\mathcal{C}]= \text{Win}_{\exists,\forall}^{T,U} (B\mathbf{\ U\ }Z)=\quad\quad\quad\quad\quad\quad\ \ \\
			\begin{cases}
			X_0 = \emptyset,\ \mathcal{V}=\{\},\ \mathcal{K}=\{\},\ {\color{purple} k=0}\\
		...
		\begin{minipage}{.5\textwidth}
			\begin{align}
			[Z_{\infty},\mathcal{C}]=\text{PGPre}_{\exists,\forall}^{T,U} (Z,B)=\quad \ \\
			\begin{cases}
...
\begin{algorithm}
	\small
	\caption{$ \text{PGPre}_{\exists,\forall}^{T,U} (Z,B) $}
	\label{alg:algorithm-label}
	\begin{algorithmic}
		\REQUIRE $ T, U, Z, B $\\
		\STATE $ Z_{\infty} = Z,\ \mathcal{V} = \{\},\ \mathcal{K}=\{\}, k = 1 $
		\FOR{$ U\in 2^U $} \FOR{$ G\in G(U) $}
		\STATE $ [V_k,\mathcal{C}_k]=Inv_{\exists}^{U,G}(Z_{\infty},B) $, $ Z_{\infty} = Z_{\infty} \cup V_k $, $ \mathcal{V}(k)=V_k,\ \mathcal{K}(k)=\mathcal{C}_k $, $ {\color{purple} k = k+1} $
		\ENDFOR
		\ENDFOR
